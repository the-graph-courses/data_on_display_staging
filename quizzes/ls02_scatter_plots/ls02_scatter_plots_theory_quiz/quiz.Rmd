...
```{r echo = F}
knitr::include_graphics(here("lessons/images/plot_types.jpg"))
```
...
A number of aesthetics can be specified inside a **`geom_*()`** function. Options for the `geom_point()` function include `color`, `size`, and `alpha`.

These control the visual elements of the points by **mapping data** variables to color, size, and transparency, respectively.
