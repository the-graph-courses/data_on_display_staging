...
# This is the working doc to use for recording. The code will be filled in and squares deleted by the end of the lesson. Always start with the blank version (ch03_intro_to_data_viz/lessons/ch03_ls01_gg_intro_prez.Rmd). No need to save this at the end, but you can if you need to make changed. Knitting will not work since chunk options are missing.
...
For now, we'll just focus on these three inputs. ![](images/gg_min_build3.png)

The other layers will be addressed in later lessons.
...
• **Mappings** always go with a **variable name** from the data frame placed **inside** `aes()` -e.g., `mapping = aes(color = VARIABLE_NAME)`.



• **Fixed aesthetics** are set to a **constant** value **outside** **`aes()`**, directly **inside the** **`geom_*()`** **function** - e.g., `geom_point(color = COLOR_NAME)`.



The differences between these methods are best understood with examples, so let's add fixed aesthetics to see how this works!
...
• Line width of all lines has increased to 1 mm.
...
::: watch-out
Let's move the `size = 1` aesthetic from `geom_line()` to `aes()` to see how it can go wrong:
...
> The transferable skills from {ggplot2} are not the idiosyncrasies of plotting syntax, but a powerful way of thinking about visualization, as a way of mapping between variables and the visual properties of geometric objects that you can perceive. - Hadley Wickham
...
5.  You can add and modify **fixed aesthetics** set to a constant value, such as color, fill, and **size**.
