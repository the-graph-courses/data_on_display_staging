...
2.  You can adjust the number or size of bins on a histogram by with **`bins`** or **`binwidth`**.
...
![](images/ggplot_cheatsheet-1.png)
In previous lessons we've used scatterplots and line graphs to visualize *relationships* between *two* continuous variables (bivariate plots), but now we are going to focus on plotting distributions of a *single* continuous variable (univariate plots). 

![](images/ggplot2_one_continuous_geoms.png)
The `geom_*()` functions we'll use in this lesson are `geom_histogram()` and `geom_density()`.
...
What happens if we try to map two variables with `geom_histogram()`?
