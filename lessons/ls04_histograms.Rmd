...
.mute(here("lessons/ls04_histograms_autograder.R"))
...
![](images/hist_dens_01.png){width="567"}

## Introduction

Histograms and kernel density estimates (KDEs) are both univariate plots that are used to visualize the distribution of continuous data. You can think of a KDE as a 'smoothed histogram'.

In this lesson we will learn how to plot them using {ggplot2}.
...
2.  Adjust the number or size of bins on a histogram by with **`bins`** or **`binwidth`**.
3.  You can plot a kernel density estimate (KDE) to visualize the distribution of continuous variables using **`geom_density()`**.
4.  You can adjust the smoothing bandwidth of a KDE with **`bw`**.
5.  You can plot a histogram and KDE on the same plot.
6.  You can use **`position`** adjustments to plot stacked, dodged, or overlapping distributions.

## Univariate distributions in {ggplot2}

The Geoms section of the {ggplot2} cheatsheet lists common plot types and the `geom_*()` functions used to create them.

![](images/ggplot_cheatsheet-1.png) In previous lessons we've used scatterplots and line graphs to visualize *relationships* between *two* continuous variables (bivariate plots), but now we are going to focus on plotting distributions of a *single* continuous variable (univariate plots).

![](images/ggplot2_one_continuous_geoms.png) The `geom_*()` functions we'll use in this lesson are `geom_histogram()` and `geom_density()`.
...
Now let's use {ggplot2} to plot the distribution of childrens' heights, which is recorded in the `height_cm` column of `malidd`.
...
::: warning
After running the previous R code, the warning message "`stat_bin()` using `bins = 30`. Pick better value with `binwidth`." is returned. This is because we have not set a bin width manually. We can ignore this for now, and we'll learn how to remove it in the next section.
:::

The y-axis of the histogram is automatically computed as follows:

1.  The x-axis is cut into a series of *bins*, where each bin represents a range of values.

2.  We then count the number of observations that fall within the range of each bin.
...
We now have an easier time associating ranges of values to each of the bins.
...
Now that we can see the bars more clearly, let's unpack the resulting histogram. Some key questions to ask are:
...
Next, change the x and y axis titles to "Age (months)" and "Number of children".
...
```{r echo=FALSE, fig.cap="Histogram with different bin settings"}
bins5 <- 
ggplot(data =  malidd , 
       mapping = aes(x = height_cm)) +
...
Now create the same plot again, but this time set the bin width to 1, and change the x and y axis labels to "Respiratory rate" and "Frequency".
...
Then, adjust the bin breaks on the x-axis to range from the lowest value of `freqrespi` to the highest, with intervals of 4.

Finally, adjust the x-axis scale breaks using a `scale_*()` function. Set the range to 24-60, with an intervals of 8.
...
### Adjusting KDE bandwidth with `bw`
...
a <- ggplot(data =  malidd, 
       mapping = aes(x = muac_cm)) +
  geom_density(color = "chartreuse4", 
               linewidth = 1,
               bw = 0.1) +
  labs(title = "bw = 0.1")
