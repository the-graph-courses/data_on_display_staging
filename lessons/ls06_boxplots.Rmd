...
![Anatomy of a boxplot](images/boxplot_anatomy.png){alt="Anatomy of a boxplot"}
...
2.  Whiskers --- Lines extending from both ends of the box indicate variability outside Q1 and Q3. The minimum/maximum whisker values are calculated as $Q1 - 1.5 \times IQR$ to $Q3 + 1.5 \times IQR$ . Everything outside is represented as an outlier.
...
-   The solid line that divides the box into 2 parts represents the [median](https://en.wikipedia.org/wiki/Median) of the data. If the median is 10, it means that there are the same number of data points below and above 10.
...
-   The difference between Quartiles 1 and 3 is called the [interquartile range](https://en.wikipedia.org/wiki/Interquartile_range) (IQR). It is a measure of the spread of the middle 50% of values, with longer boxes indicating more variability.

-   The extreme line (or "whisker") shows $Q1 - 1.5 \times IQR$ to $Q3 + 1.5 \times IQR$ (the lowest to highest values, excluding outliers).

-   Dots (or other markers) represent values that fall beyond the extremes of the whiskers and are plotted individually. These are called *outliers* and they can be thought of as anomalous ("out-of-the-ordinary") values.
...
This time, we will use boxplots display and compare *distributions* of variables across multiple groups.
...
Let start with a base boxplot and then then add more aesthetics and layers from {ggplot2}.
