...
• First we will variables to an aesthetic inside `aes()`
...
• Gradual, continuous color scaling • The gradient communicates continuum of values • But `breastfeeding` has only two distinct values: 0 or 1
...
• The 1s and 0s represent two *discrete* possibilities • Continuous color scaling is not suitable
...
Adding colors let us to visualize a **third variable**
...
```{r}
# Submit your answer:
age_height_respi <- ggplot(data = nigerm04, 
       mapping = aes(x = week, 
                     y = cases)) +
  geom_point()
...
```{r}
# Submit your answer:
age_height_fever <- ggplot(data = nigerm04, 
       mapping = aes(x = week, 
                     y = cases)) +
  geom_point()
...
• We can see with points overlap • `alpha` is a useful aesthetic modifiction when there is **overplotting**
...
```{r}
# Submit your answer:
age_viral_blue <- ggplot(data = nigerm04, 
       mapping = aes(x = week, 
                     y = cases)) +
  geom_point()
...
• The smoothing line comes os overlaid on top of the points.



• The smoothing function here is "loess" which stands for for **l**ocally **w**eighted **scatter plot** **s**moothing
...
• The blue points mostly occur above the trend line • Children with higher viral loads were more likely to exhibit symptoms of vomiting
...
```{r}
# Submit your answer:
age_height_2 <- ggplot(data = nigerm04, 
       mapping = aes(x = week, 
                     y = cases)) +
  geom_point()
...
```{r}
# Submit your answer:
age_height_3 <- ggplot(data = nigerm04, 
       mapping = aes(x = week, 
                     y = cases)) +
  geom_point()
